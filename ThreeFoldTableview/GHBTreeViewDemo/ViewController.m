/**
     ...
     1，只有一个section
     2，由数据控制列表的展示
     3，该demo中tableview有三级列表，设置了三种类型的cell。
     4，每次点击cell的时候，更新数据的折叠状态。（如果是最里层的cell，则执行其他操作。）
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (node.nodeType == 2) {
        //处理子节点选中，此处自己定义(点击了联系人会来这里)
        
    }else{
        
        //点击nodeType为0和1时，旋转按钮。
...
- (void)addTestData{
    ...
    node0.nodeLevel = 0;    //节点所在层次
    node0.nodeType = 0;     //节点类型
    ...
    node3.sonNodes = [NSMutableArray arrayWithObjects:node5,node7, nil];
    node4.sonNodes = [NSMutableArray arrayWithObjects:node6, nil];
//    node2.sonNodes = [NSMutableArray arrayWithObjects:node10, nil];
